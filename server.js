const express = require('express');
     ...
     app.use((req, res, next) => {
         req.session = sessions[req.headers['x-session-id']] || { isAuthenticated: false };
         console.log(`[${new Date().toISOString()}] 請求: ${req.method} ${req.path}, Session ID: ${req.headers['x-session-id'] || 'none'}, isAuthenticated: ${req.session.isAuthenticated}`);
         next();
     });
     ...
     app.post('/logout', (req, res) => {
         console.log(`[${new Date().toISOString()}] 登出請求，Session ID: ${req.headers['x-session-id']}`);
     ...
     app.post('/add-product', isAuthenticated, (req, res) => {
         try {
             ...
             if (!name || !price || !img) {
                 return res.status(404).json({ success: false, message: '產品名稱、價格和圖片為必填' });
             }
             const product = {
